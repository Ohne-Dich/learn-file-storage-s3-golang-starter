...
func main() {
	...
	default_aws_sdk_cfg, err := config.LoadDefaultConfig(context.Background(), config.WithRegion(s3Region))
	if err != nil {
		log.Fatalf("Unable to get dafault aws sdk config: %v", err)
	}

	s3Client := s3.NewFromConfig(default_aws_sdk_cfg)
